...
int main() {
    ...
    if (mkdir_status != 0) {
        fprintf(stderr, "Failed to create directory %s\n, please make a github issue", DEST_DIR);
        return 1;
    }
    ...
    if (mv_status != 0) {
        fprintf(stderr, "Failed to move script to %s\n, please make a github issue", DEST_PATH);
        return 1;
    }

    printf("! qfcd() function installed to %s\n", DEST_PATH);
    printf("======= It will now load in all POSIX login shells (bash, zsh, sh, etc. [anything which has sh in the name :)])\n =======");
